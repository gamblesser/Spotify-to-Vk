...
def find_element(xpath: str,driver, timeout: int = 10) -> WebElement or None:
    try:
...
def writeToFile(file_name):
	...
	with open(f'{winshell.desktop()}\\{file_name}.txt', 'w', encoding="utf-8") as fp:
		for song in songsForYoutube:

			fp.write(f"{song}\n")

	with open(f'{winshell.desktop()}\\{file_name}_ForYandex.txt', 'w', encoding="utf-8") as fp:
...
def getSongsFromSpotify(url):
	...
	with webdriver.Chrome(PathToChromeWebDriver,options=options, desired_capabilities=capabilities) as driver:
		...
		title =find_element('//span/h1',driver).text
		songs=find_element('//div[@role="presentation"][2]',driver)

		maxHeight= driver.execute_script("return document.getElementsByClassName('os-viewport')[0].scrollHeight")

		while True:
			try:
				find_element(f'//div[@aria-rowindex="{songCursor}"]',songs,timeout=2)
			except:
				notFound=True
			...
			if notFound:
				while True:
					if theEnd:
						...
						print('\n'.join(listForYoutube))
						print("Length: ",len(listForYoutube))
					...
					songs=find_element('//div[@role="presentation"][2]',driver,timeout=2)
					try:
						find_element(f'//div[@aria-rowindex="{songCursor}"]',songs,timeout=2)

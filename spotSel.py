...
class CollectTrecksFromSpotify:
    ...
    def __call__(self, url):
        self.browser = webdriver.Chrome(self.pathToChromeWebDriver,
    ...
    def getSongsFromSpotify(self):
        ...
        maxHeight = self.browser.execute_script(
            "return document.getElementsByClassName('os-viewport')[0].scrollHeight"
        )

        while True:
            try:

                self.__find_element(f'//div[@aria-rowindex="{songCursor}"]',
                                    songs,
                                    timeout=2)
            ...
            if notFound:
                while True:
                    ...
                    self.browser.execute_script(
                        f"document.getElementsByClassName('os-viewport')[0].scrollTo(0,{height})"
                    )
                    songs = self.__find_element(
                        '//div[@role="presentation"][2]', timeout=2)

                    try:
                        self.__find_element(
                            f'//div[@aria-rowindex="{songCursor}"]',
                            songs,
                            timeout=2)
            ...
            try:
                ...
                self.listForYoutube.append(' - '.join([
                    title.text.replace('\n', '').strip()
    ...
    def __writeToFile(self):
        ...
        with open(f'{winshell.desktop()}\\{self.title}.txt',
                  'w',
        ...
        with open(f'{winshell.desktop()}\\{self.title}_ForYandex.txt',
                  'w',
                  encoding="utf-8") as fp:
    ...
    def __find_element(self,
                       xpath: str,
                       node=None,
                       timeout: int = 15) -> WebElement or None:
        try:
            node = self.browser
            if node:
                node = node
            element = WebDriverWait(node, timeout).until(
